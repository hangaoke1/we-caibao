...
export default {
  ...
  computed: {
    ...
    baseCount () {
      ...
      let v = 1;
      this.info.data.forEach(match => {
        v = v * match.choose.length
      })
      return v
    },
    selectCount() {
  ...
  methods: {
    ...mapActions({
      updateToto14: 'updateToto14',
      updateBalanceInfo: 'updateBalanceInfo',
      updateCoupons: 'updateCoupons'
    }),
    ...mapMutations(['updateToto14ChooseByIndex', 'emptyJToto14Choose', 'updateChooseCouponId']),
    goChooseCoupon() {
    ...
    async handleSubmit() {
      try{
        ...
        // 跳转
        uni.navigateTo({
          url: `/pages/order/success?schemeId=${res.schemeId}&lotteryId=10039`
        });
      }catch(e){
    ...
    chooseChange(match, choose, index) {
      ...
      this.updateToto14ChooseByIndex({
        choose: newChoose,
        index
      })
    }
