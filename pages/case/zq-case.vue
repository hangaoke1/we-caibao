    ...
    <g-tr>
      <g-td col="150" padding="0">
        <view class="u-td-wrap">
          <text class="f-30">场次</text>
        </view>
      </g-td>
      <g-td col="240" padding="0">
        <view class="u-td-wrap">
          <text class="f-30">主队对客队</text>
        </view>
      </g-td>
      <g-td col="240" padding="0">
        <view class="u-td-wrap">
          <text class="f-30">投注项</text>
        </view>
      </g-td>
      <g-td col="120" padding="0">
        <view class="u-td-wrap">
          <text class="f-30">彩果</text>
        </view>
      </g-td>
    </g-tr>
    <g-tr v-for="(match, index) in matches" :key="index">
      <g-td col="150" padding="0">
        <view class="u-td-wrap" style="flex-direction: column;">
          <text
            class="f-30 iconfont"
            :class="[match.matchId ? 'red-6': 'grey-1']"
            @click.stop="goH5(match)"
          >&#xe609;</text>
      ...
      <g-td col="240" padding="0">
        <view class="u-td-wrap" style="flex-direction: column;" v-if="match.openFlag">
          ...
          >
            <text v-if="[403, 401, 400].includes(choose) && match.rate">({{match.rate}})</text>
            <text>{{ jczqMap[choose].name }}</text>
            <text v-if="match.odds">[{{match.odds[index] }}]</text>
            <text v-else>[-]</text>
          </text>
        </view>
