  ...
  <view class>
    <view class="PinTu">
      <view
        class="item"
        :class="item.class"
        :style="[itemStyle,{'background-image': 'url('+url+')'}]"
        v-for="(item,index) in levelArray"
        :data-index="index"
        :key="index"
        @tap="change"
      ></view>
    ...
    <view class="btn-group">
      <u-button type="primary" @tap="start">{{startText}}</u-button>
      <u-button type="error" @tap="back">结束游戏</u-button>
    </view>
  </view>
</template>
  ...
  methods: {
    ...
    change(e) {
      ...
      if (that.gameStart && emptyNum != that.levelArray[ele].value) {
        let topNum = that.levelArray[ele - level]
          ? that.levelArray[ele - level].value
          : -1;
        let leftNum = that.levelArray[ele - 1]
          ? that.levelArray[ele - 1].value
          : -1;
        let rightNum = that.levelArray[parseInt(ele) + 1]
          ? that.levelArray[parseInt(ele) + 1].value
          : -1;
        let bottomNum = that.levelArray[parseInt(ele) + parseInt(level)]
          ? that.levelArray[parseInt(ele) + parseInt(level)].value
          : -1;
        let temp = {};
        ...
        switch (emptyNum) {
          ...
          case bottomNum: {
            temp = that.levelArray[ele];
            that.levelArray[ele] =
              that.levelArray[parseInt(ele) + parseInt(level)];
